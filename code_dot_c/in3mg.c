...
if (sa[0]!='0')
	...
	/* Node  Cycle */
/*
#pragma omp parallel for private(m1,m2,m3,mgi,mcmax1,eps1,ui1,un1) shared(vx,vy,vz,pr,sxx,syy,szz,sxy,sxz,syz,exx,eyy,ezz,exy,exz,eyz) firstprivate(mgx,mgy,mgz,mgxy,mgp)
*/
	...
	/**/
	/**/
	/* Compute marker strain rates */
/*
#pragma omp parallel for shared(markx,marky,markz,val1) private(wn1,eps1,mm1) firstprivate(marknum,xsize,ysize,zsize) schedule(static)
*/
	...
	if(markx[mm1]>=0 && marky[mm1]>=0 && markz[mm1]>=0 && (double)(markx[mm1])<=xsize && (double)(marky[mm1])<=ysize && (double)(markz[mm1])<=zsize)
		...
		epscalc1(markx[mm1],marky[mm1],markz[mm1],0,eps1,wn1);
